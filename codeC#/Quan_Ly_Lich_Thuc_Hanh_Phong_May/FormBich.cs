    ...
    public partial class FormBich : Form
    {
        ...
        private void LoadThongTinLTH()
        {
            ...
            if (dataGridViewLTH.CurrentRow != null)
            {
                ...
                tenGiangVien = row.Cells["TenGV"].Value?.ToString();
                soBuoi = row.Cells["SoBuoi"].Value?.ToString();
                tenLopHoc = row.Cells["TenLop"].Value?.ToString();
            ...
            f.Location = new Point(730, 13);

            page_lth.Controls.Add(f);
            f.Show();
            f.LoadThongTin(maLich, tenGiangVien, soBuoi, tenLopHoc, tenMonHoc, ngayDK, hocKy, namHoc);
        }
        ...
        private void LoadThongTinPTT()
            {
                ...
                string noiDungTH = row.Cells["NoiDungTH"].Value?.ToString() ?? "";
                string hstt = row.Cells["HeSoTaiTruc"].Value?.ToString() ?? "";
                string soGio = row.Cells["SoGio"].Value?.ToString() ?? "";
                string loaiGT = row.Cells["LoaiGioTruc"].Value?.ToString() ?? "";
                string ngayTH = row.Cells["NgayTH"].Value?.ToString() ?? "";
                string gioBD = row.Cells["GioBD"].Value?.ToString() ?? "";
                string gioKT = row.Cells["GioKT"].Value?.ToString() ?? "";

                // Cấu hình form con
                p.FormClosed += p_FormClosed;
                p.TopLevel = false;
                p.FormBorderStyle = FormBorderStyle.None;
                p.Location = new Point(730, 10);

                page_PTT.Controls.Add(p);
        
                p.SetDataFromOtherForm(maLich, tenNV, tenPhong, soPhieu, noiDungTH, ngayTH, gioBD, gioKT, loaiGT, hstt, soGio);
                p.LoadThongTin();
        ...
        private void FormBich_Load(object sender, EventArgs e)
        {
            ...
            //tabControl1.ItemSize = new Size(0, 15);
        ...
        private void bich_btn_dklth_Click(object sender, EventArgs e)
        {
            ...
            p.FormClosed += p_FormClosed;
            p.FormBorderStyle = FormBorderStyle.None;
            p.Location = new Point(700, 13); // Vị trí
        ...
        private void bich_btn_capnhat_Click(object sender, EventArgs e)
        {
            ...
            f1.Show();
            f1.SetThongTin(maLich, tenGiangVien, soBuoi, tenLopHoc, tenMonHoc, ngayDK, hocKy, namHoc);
            LoadDanhSachLTH();
        }
        ...
        private void cbLocLTH_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (selectedField == "Tên giảng viên")
            {
                cbDataLocLTH.Text = "Chọn giảng viên";
            ...
            else if (selectedField == "Tên lớp học")
            {
                cbDataLocLTH.Text = "Chọn lớp học";
                SqlDataReader readerGV = db.ExecuteReader("SELECT TenLop FROM LopHoc WHERE MaLop <> '----'");
                if (readerGV != null)
                {
                    while (readerGV.Read())
                    {
            ...
            {
                cbDataLocLTH.Text = "Chọn môn học";
                SqlDataReader readerGV = db.ExecuteReader("SELECT TenMH FROM MonHoc WHERE MaMH <> '----'");
                if (readerGV != null)
                {
                    while (readerGV.Read())
                    {
        ...
        private void bich_btn_loc_Click(object sender, EventArgs e)
        {
            ...
            string sql = "";
            if (s1 == "Tên giảng viên")
            {
                sql = "select MaLich, SoBuoi, TenGV, TenLop, TenMH, HK, NamHoc, NgayDK " +
                    "from LichTh join MonHoc on LichTh.MaMH = MonHoc.MaMH " +
                    "join GiangVien on GiangVien.MaGV = LichTH.MaGV " +
                    "join LopHoc on LopHoc.MaLop = LichTH.MaLop " +
                    "where TenGV = @s2";
            }
            else if (s1 == "Tên lớp học")
            {
                sql = "select MaLich, SoBuoi, TenGV, TenLop, TenMH, HK, NamHoc, NgayDK " +
                    "from LichTh join MonHoc on LichTh.MaMH = MonHoc.MaMH " +
                    "join GiangVien on GiangVien.MaGV = LichTH.MaGV " +
        ...
        private void cbLocPTT_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            { 
                SqlDataReader readerGV = db.ExecuteReader("SELECT MaLich FROM LichTH WHERE MaLich <> '----'");
                if (readerGV != null)
                {
                    cbDataLocPTT.Text = "Chọn lịch";
                    while (readerGV.Read())
                    {
            ...
            else if (selectedField == "Tên nhân viên")
            {
                SqlDataReader readerGV = db.ExecuteReader("SELECT TenNV FROM NhanVien WHERE MaNV <> '----'");
                if (readerGV != null)
                {
            ...
            else if (selectedField == "Tên phòng máy")
            {
                SqlDataReader readerGV = db.ExecuteReader("SELECT TenPhong FROM PhongMay WHERE MaPhong <> '----'");
                if (readerGV != null)
                {
                    cbDataLocPTT.Text = "Chọn phòng máy";
                    while (readerGV.Read())
                    {
                        cbDataLocPTT.Items.Add(readerGV.GetString(0));
                    }
        ...
        private void btnLocPTT_Click(object sender, EventArgs e)
        {
            ...
            if (s1 == "Mã lịch")
            {
                sql = "select SoPhieu, MaLich, TenPhong, TenNV, LoaiGioTruc, " +
                    "CONVERT(VARCHAR(5), GioBD, 108) AS GioBD, " +
                    "CONVERT(VARCHAR(5), GioKT, 108) AS GioKT, " +
                    "CONVERT(VARCHAR(10), NgayTH, 103) AS NgayTH, NoiDungTH, " +
                    "CAST(HeSoTaiTruc AS DECIMAL(10,2)) AS HeSoTaiTruc, " +
                    "CAST(SoGio AS DECIMAL(10,4)) AS SoGio  " +
                    "from PhieuTaiTruc join Nhanvien on PhieuTaiTruc.MaNV = NhanVien.MaNV " +
                    "join PhongMay on PhieuTaiTruc.MaPhong = PhongMay.MaPhong where MaLich = @s2 " +
                    "ORDER BY SoPhieu";
            }
            else if (s1 == "Tên nhân viên")
            {
                sql = "select SoPhieu, MaLich, TenPhong, TenNV, LoaiGioTruc, " +
                    "CONVERT(VARCHAR(5), GioBD, 108) AS GioBD, " +
                    "CONVERT(VARCHAR(5), GioKT, 108) AS GioKT, " +
                    ...
                    "CAST(SoGio AS DECIMAL(10,4)) AS SoGio  " +
                    "from PhieuTaiTruc join Nhanvien on PhieuTaiTruc.MaNV = NhanVien.MaNV " +
                    "join PhongMay on PhieuTaiTruc.MaPhong = PhongMay.MaPhong where TenNV = @s2 " +
                    "ORDER BY SoPhieu";
            }
            else if (s1 == "Tên phòng máy")
            {
                sql = "select SoPhieu, MaLich, TenPhong, TenNV, LoaiGioTruc, " +
                    "CONVERT(VARCHAR(5), GioBD, 108) AS GioBD, " +
                    "CONVERT(VARCHAR(5), GioKT, 108) AS GioKT, " +
                    ...
                    "CAST(SoGio AS DECIMAL(10,4)) AS SoGio  " +
                    "from PhieuTaiTruc join Nhanvien on PhieuTaiTruc.MaNV = NhanVien.MaNV " +
                    "join PhongMay on PhieuTaiTruc.MaPhong = PhongMay.MaPhong where TenPhong = @s2 " +
                    "ORDER BY SoPhieu";
            }
            try
            {
        ...
        private void ptt_btnDK_Click(object sender, EventArgs e)
        {
            ...
            ptt.Location = new Point(730, 10); // Vị trí
        ...
        private void ptt_btnUpd_Click(object sender, EventArgs e)
        {
            ...
            if (dataGridViewPTT.CurrentRow != null)
            {
                DataGridViewRow row = dataGridViewPTT.CurrentRow;
                maLich = row.Cells["MaLich"].Value?.ToString() ?? "";
                maPhong = row.Cells["TenPhong"].Value?.ToString() ?? "";
                maNV = row.Cells["TenNV"].Value?.ToString() ?? "";
            ...
            p1.SetDataFromOtherForm(maLich, maNV, maPhong,soPhieu, noiDungTH, ngayTH, gioBD,gioKT,loaiGT, hstt,soGio);
            bool check = p1.SetThongTin();
            if (check)
            {
                p1.Show();
            }
        ...
        private void XoaPhieuTaiTruc(int soPhieu)
        {
            try
            {
        ...
        private void ptt_btnXoa_Click(object sender, EventArgs e)
        {
            ...
            if (result == DialogResult.Yes)
            {
                ...
                {
                    soPhieu = Convert.ToInt32(row.Cells["SoPhieu"].Value.ToString() ?? "");
                    XoaPhieuTaiTruc(soPhieu);
                }
                LoadDanhSachPTT();
            }
        }
        ...
        private void ptt_btnTK_Click(object sender, EventArgs e)
        {
            ...
            f1.TimKiemXong += CapNhatKetQuaTimKiemPTT;
            f1.TopLevel = false;
            f1.FormBorderStyle = FormBorderStyle.None;
            f1.Location = new Point(730, 10);
