...
namespace Quan_Ly_Lich_Thuc_Hanh_Phong_May
{
    public partial class PhieuTaiTruc : Form
    {
        ...
        private void LoadDataToComboBoxes()
        {
            ...
            SqlDataReader readerLop = db.ExecuteReader("SELECT TenNV FROM NhanVien where MaNV != '----'");
            if (readerLop != null)
            {
            ...
            SqlDataReader readerMH = db.ExecuteReader("SELECT TenPhong FROM PhongMay where MaPhong != '----'");
            if (readerMH != null)
            {
                while (readerMH.Read())
                {
        ...
        private void ResetForm()
        {
            ...
            cbMaNV.Text = "Chọn tên nhân viên";
            cbMaPM.Text = "Chọn tên phòng máy";
            txtNoiDung.Text = "";
            rdbHC.Checked = false;
            rdbGN.Checked = false;
    ...
    DateTime ngay, TimeSpan gioBD, TimeSpan gioKT, string loaiGioTruc)
        {
            try
            {
                using (SqlCommand cmd = new SqlCommand("bich_xepLichTruc", db.Connection))
                {
                    ...
                    cmd.Parameters.AddWithValue("@tenPhong", maPhong);
